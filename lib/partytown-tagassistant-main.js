!function(win, config) {
  ...
  const tunnel = win.__GTM_DEBUG_QUEUE_TUNNEL = {
    ...
    pushToQueue: function(...args) {
      ...
      if(tagAssistant.debug) console.log('[PT GTM Debug Queue - Main] pushToBootstrapQueue called. Args:', args);
  ...
  function testStringAgaistPatterns(patterns, value) {
    return (
      patterns
  ...
  function shouldProcessScript(src) {
    return src && testStringAgaistPatterns(scriptsToMonitor, src);
  }
  ...
  const checkAndForcePartytownExecution = function(element, currentSrc) {
    ...
    if (shouldProcessScript(currentSrc)){
      ...
      } else if (!testStringAgaistPatterns(config.loadScriptsOnMainThread, realSrc)) {
